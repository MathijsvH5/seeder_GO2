...
public:
  ...
  void RobotControl()
  {
    ...
    if (ct < 2.0) {
        ...
        // Phase 2: Switch to Active Balance (The "Handshake")
        TEST_MODE = balance_stand;
    }
    else if (ct < 7.0) {
        // 0 to 3 seconds: Walk forward
        TEST_MODE = velocity_move;
        current_vx = 0.6;   // 0.3 m/s forward
        current_vyaw = 0.0; // No turning
    } 
    else if (ct < 7.5) {
        // BUFFER 1: Stop and stabilize
        TEST_MODE = stop_move;
    }
    else if (ct < 11.0) {
        // 3 to 7 seconds: Turn 180 degrees in place
        TEST_MODE = velocity_move;
        current_vx = 0.0;     // Stop moving forward
        current_vyaw = 1; // Turn at 0.785 rad/s (approx 180 degrees over 4 seconds)
    }
    else if (ct < 11.5) {
        // BUFFER 2: Stop and stabilize
        TEST_MODE = stop_move;
    } 
    else if (ct < 14.5) {
        // 7 to 10 seconds: Walk forward again
        TEST_MODE = velocity_move;
        current_vx = 0.6;   // 0.3 m/s forward
        current_vyaw = 0.0; // No turning
    } 
    else if (ct < 15.0) {
        // BUFFER 3: Stop and stabilize
        TEST_MODE = stop_move;
    }
    else if (ct < 16.0){
        // After 10 seconds: Sit down and stay seated
        TEST_MODE = stand_down;
    }
    else {
        keep_running = false;      // >10s: Trigger safe script exit
    }
    ...
    case height_control:
     // sport_client.BodyHeight(current_height); // Requires patched SDK
      sport_client.Euler(0, 0, 0);             // Keep it level
      sport_client.BalanceStand();
      break;
...
int main(int argc, char **argv)
{
  ...
  // --- NEW EXIT LOGIC ---
  // The loop runs until keep_running is set to false in RobotControl
  ...
  return 0; // Kills the recurrent thread and gracefully exits sport mode
}