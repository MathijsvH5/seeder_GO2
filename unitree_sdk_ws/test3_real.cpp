...
// Add this right before "class Custom {"
class MySportClient : public unitree::robot::go2::SportClient 
{
public:
    int32_t SetBodyHeight(float height) 
    {
        std::string parameter = "{\"height\":" + std::to_string(height) + "}";
        
        // This will show up in your terminal every time the command is sent
        // We use a small check so it doesn't spam too fast
        static int count = 0;
        if (count++ % 100 == 0) {
             std::cout << "Sending Height Command: " << parameter << std::endl;
        }

        return Call(1013, parameter); 
    }
};
  ...
  void RobotControl()
  {
    ...
    case height_control:
      if (TEST_MODE != last_mode) {
          sport_client.Pose(true); // Tell the robot we want to control its posture
      }
      sport_client.SetBodyHeight(-0.15f);
      break;
