...
public:
  ...
  Custom() {
    ...
    sequence.push({stand_up,      2.0}); // 3. Stand up (Allow time to finish move)
    sequence.push({seeder_next,   2.0, 0.0, 0.0, 0.0}); // 2. Send 'NEXT' (while down)
    sequence.push({pitch_control, 2.0, 0.0, 0.0, 0.4}); 
    sequence.push({seeder_open,   2.0, 0.0, 0.0, 0.4}); // 2. Send 'OPEN' (while down)
    sequence.push({pitch_control, 2.0, 0.0, 0.0, 0.0}); 
    sequence.push({seeder_close,  2.0, 0.0, 0.0, 0.0}); // 4. Send 'CLOSE' (while standing)
    sequence.push({stand_down,    1.0}); // 5. Lay back down
  ...
  bool SendMessageToESP32(const std::string& msg) {
      ...
      while (read(serial_port, trash_buffer, sizeof(trash_buffer)) > 0) {
          // Keep reading until the buffer is empty (returns -1 with EAGAIN)
      }
      ...
      if (bytes_written < 0) {
          // Only print the error once per state to avoid spamming 200Hz
